...
const PopupModal:React.FC<ModalProps> = ({setModal, navigate}) => {
  ...
  return (
    <Modal  setModal={setModal} title={"Choose Login As"}>
        ...
        <Button className="btn-primary text-white" onClick={() => SuccessResponse(hrDemoData, "demo token", "HR")}>
            HR
          </Button>
          <Button className="btn-ghost" onClick={() => SuccessResponse(empDemoData, "demo token", "Employee")}>
            Employee
          </Button>
        </div>
    </Modal>
